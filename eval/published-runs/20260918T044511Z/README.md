...
1. Đọc `../../archive/transcript-review-worksheet.md` và `../../archive/golden-transcript.json` để xem expected, required_claims và forbidden_claims.
...
Lệnh export không gọi API; cập nhật `eval/archive/transcript-run-results.md` theo phiếu chấm. Không đổi expected/quality bar sau khi đọc output. 24/24 action đúng là số kỹ thuật; full pass rate vẫn pending đến khi mọi case được chấm. Dữ liệu này chỉ là một lượt dùng Gemini qua proxy, không phải kết quả của model/provider khác.
