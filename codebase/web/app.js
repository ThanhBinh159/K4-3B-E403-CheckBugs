...
function renderPages() {
  ...
  for(const row of rows) {
    ...
    check.onchange=()=>{ if(check.checked&&selected.size>=3){check.checked=false;error('Bạn chỉ có thể chọn tối đa 3 trang.');return;} check.checked?selected.add(row.id):selected.delete(row.id);error('');selectedInfo(); };
...
async function openSegment(sourceId,id) {
  ...
  try {
    ...
    $('sourceView').hidden=false;$('sourceView').scrollIntoView({behavior:'smooth',block:'nearest'});
...
$('source').onchange=async()=>{
  ...
  show('idle','Đặt câu hỏi theo bài đã chọn.','Câu hỏi của bạn được giữ lại. Lượt hỏi tiếp theo sẽ dùng bộ slide mới.');
...
$('preview').onclick=()=>{if(selected.size)openSegment($('source').value,[...selected][0]);};
$('clearPages').onclick=()=>{selected.clear();renderPages();};
$('closeSource').onclick=hideSource;
$('question').oninput=()=>{$('counter').textContent=$('question').value.length+' / 2.000';error('');};
...
function editQuestion(){$('question').focus();$('question').select();$('question').scrollIntoView({behavior:'smooth',block:'center'});}
$('askForm').onsubmit=async event=>{
  ...
  if(!input.source_id){error('Chọn bộ slide bài học trước khi hỏi tutor.');$('source').focus();return;}
  if(!input.question){error('Nhập câu hỏi bạn muốn tìm hiểu.');$('question').focus();return;}
  error('');lock(true);show('loading','Tutor đang đọc phần nguồn liên quan…','Đang tạo lời giải thích cho câu hỏi của bạn. Nếu nguồn chưa đủ hoặc câu hỏi chưa rõ, tutor sẽ gợi ý bước tiếp theo.');
  try{
    ...
    if(value.action!=='answer')button('actions','Chọn trang hoặc đổi bài',()=>{$('source').focus();$('source').scrollIntoView({behavior:'smooth',block:'center'});});
